...
let WindInformationHandle = null
export const Windy = function (params) {
  ...
  var MAX_WIND_INTENSITY = 10 // 40      // wind velocity at which particle intensity is maximum (m/s)
  ...
  var createField = function (columns, colums_orgs, bounds, callback) {
    store.commit('wind_field', columns)
    /**
    ...
    field.release = function () {
      columns = []
      colums_orgs = []
      // store.state.WindInformation.wind_field = []
    }
  ...
  var interpolateField = function (grid, bounds, extent, callback) {
    ...
    if (store.state.mapview.scale > 9244648.868618) {
      velocityScale = VELOCITY_SCALE * 0.5
    }
    ...
    function interpolateColumn (x) {
        ...
        if (coord) {
          ...
          if (isFinite(λ)) {
            ...
            if (wind) {
              ...
              var w2 = wind[1]
              var w3 = wind[2]
              colums_org[y + 1] = colums_org[y] = [w1, w2, w3]
      ...
      colums_orgs[x + 1] = colums_orgs[x] = colums_org
    }
    ...
    (function batchInterpolate () {
      ...
      createField(columns, colums_orgs, bounds, callback)
      // 风场数据获取
      store.commit('wind_field', colums_orgs)
    })()
  }
  ...
  var animate = function (bounds, field) {
    ...
    function windIntensityColorScale (step, maxWind) { // 将风场分为不同类别颜色  maxWind=40
      /*
            ...
            */
      var result = result = ['rgba(' + hexToR('#6687F7') + ', ' + hexToG('#6687F7') + ', ' + hexToB('#6687F7') + ', ' + 0.5 + ')']
      // 地形图模式切换风的颜色
      ...
      if (store.state.mapview.map.basemap.id == 'myVectorBasemap2' || store.state.mapview.map.basemap.id == 'myVectorBasemap') {
        // result = ['rgba(' + hexToR('#5b83ed') + ', ' + hexToG('#5b83ed') + ', ' + hexToB('#5b83ed') + ', ' + 0.5 + ')']
        result = ['rgba(' + hexToR('#6687F7') + ', ' + hexToG('#6687F7') + ', ' + hexToB('#6687F7') + ', ' + 0.5 + ')']
      ...
      if (store.state.mapview.map.basemap.id == 'myImageBasemap' || store.state.mapview.map.basemap.id == 'myBlackBlueBasemap') {
        result = ['rgba(' + hexToR('#FFFFFF') + ', ' + hexToG('#FFFFFF') + ', ' + hexToB('#FFFFFF') + ', ' + 0.8 + ')']
      }
    ...
    // PARTICLE_MULTIPLIER = store.state.mapview.zoom * 0.0001;//控制不同比例尺显示网格数量
    // var particleCount = Math.round(bounds.width * bounds.height * PARTICLE_MULTIPLIER)
    // particleCount = 1050
    var particleCount = 500
    // console.log("windscale:"+mapView.scale)
    // if (isMobile()) {
  ...
  var start = function (bounds, width, height, extent) {
    ...
    if (myWindy.params.canvas) {
      var g = myWindy.params.canvas.getContext('2d')
    ...
    // build grid
    // 数据格式为u和v两个元素
    buildGrid(myWindy.params.data, function (grid) {
      // interpolateField
  ...
  var stop = function () {
    ...
    store.commit('windblock_show', false)
  }
  ...
  loadModules(['esri/geometry/support/webMercatorUtils'])
    .then(([webMercatorUtils]) => {
      ...
      if (!windCanvas) {
        ...
        if (myWindy) {
          var params = { canvas: windCanvas, data: windData, mapView: curMapView }
          myWindy.params = params
        }
      }
      /*
      ...
      redraw()
      // pointer-move
      // WindInformationHandle = store.state.mapview.on('pointer-move', function (evt) {
      //     // console.log(evt.x+","+evt.y+";");
      //     let x = evt.x;
      //     let y = evt.y;
      //     let width = store.state.mapview.width;
      //     let height = store.state.mapview.height;
      //     GetWindInformation(x, y, width, height);
      //     let x_local = x - 130;
      //     let y_local = y - 120;
      //     if (y_local < 0) {
      //         y_local = y_local + 120;
      //     }
      //     $("#WindInformation").css("left", x_local + "px");
      //     $("#WindInformation").css("top", y_local + "px");
      // });
    })
}
export const clearWindCanvas = function () {
  ...
  if (windCanvas) {
    windCanvas.style.top = '0px'
    windCanvas.style.left = '0px'
  }
...
export const addWindHandle = function () {
  ...
  windMapZoomHandle = store.state.mapview.watch('zoom', function (newValue, oldValue) {
    // console.log("zoom newValue:"+newValue);
    // console.log("zoom oldValue:"+oldValue);
    clearWindCanvas()
  })
  windExtentHandle = store.state.mapview.watch('interacting', function (response) {
    if (response) {
      clearWindCanvas()
      // console.log("interacting in progress");
      if (windStationaryHandle) {
        windStationaryHandle.remove()
      }
    } else {
      // console.log("interacting stationary");
      windStationaryHandle = store.state.mapview.watch('stationary', newValue => {
        // console.log("interacting stationary");
        let windCanvas = document.getElementById('wind-canvas')
        if (windCanvas && myWindy) {
...
export const removeWindHandle = function () {
  ...
  if (WindInformationHandle) {
    WindInformationHandle.remove()
  }
}
// 根据光标推算风场数据函数
export const GetWindInformation = function (x, y, width, height) {
  var Wind_speed = 0
  var wind_direction = '无'
  var mycolumns = store.state.WindInformation.wind_field
  if (mycolumns.length !== 0) {
    ...
    store.commit('Wind_speed', Wind_speed)
    store.commit('wind_direction', wind_direction)
    store.commit('windblock_show', true)
  } else {
    store.commit('Wind_speed', 0)
    store.commit('wind_direction', '无')
    store.commit('windblock_show', true)
  }
