...
export const Windy = function (params) {
  ...
  var animate = function (bounds, field) {
    ...
    function windIntensityColorScale (step, maxWind) { // 将风场分为不同类别颜色  maxWind=40
      ...
      result = ['rgba(' + hexToR('#ffffff') + ', ' + hexToG('#ffffff') + ', ' + hexToB('#ffffff') + ', ' + 0.5 + ')']
      // 地形图模式切换风的颜色
...
export const addWindHandle = function () {
  ...
  store.state.mapview.on('double-click', function (event) {
    console.log('双击')
    if (store.state.mapview.zoom + 1 <= store.state.mapview.constraints.maxZoom) {
      store.state.mapview.zoom = store.state.mapview.zoom + 1
    }
  })
  // 监听地图是否静止
  ...
  */
  ...
  windMouseDownHandle = store.state.mapview.on('pointer-down', function (event) {
    console.log('按下鼠标')
    clearWindCanvas()
    /*
   if(myWindy){
